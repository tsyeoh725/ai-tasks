...
function ClientAvatar({ client, size = "md" }: {
  ...
  if (client.logoUrl) {
    return (
      <div className={cn("relative rounded-lg overflow-hidden shrink-0 ring-1 ring-gray-200", dim)}>
        <NextImage
          src={client.logoUrl}
          alt={client.name}
          fill
      ...
      <div className="flex items-center gap-1 shrink-0">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-44">
            ...
            <DropdownMenuItem onClick={() => (window.location.href = `/clients/${client.id}`)}><Pencil size={13} className="mr-2" /> Open dashboard</DropdownMenuItem>
            <DropdownMenuSeparator />
