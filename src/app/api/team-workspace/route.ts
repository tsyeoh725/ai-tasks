...
export async function PATCH(req: Request) {
  ...
  if (!existing) {
    await db.insert(teamWorkspace).values({
      id: uuid(),
      userId: user.id!,
      character: asString(body.character, "dev_1"),
      characterColor: asString(body.characterColor, "#99ff33"),
      x: typeof body.x === "number" ? body.x : 5,
      y: typeof body.y === "number" ? body.y : 5,
      statusEmoji: asString(body.statusEmoji, "💻"),
      statusText: asNullableString(body.statusText),
      isOnline: typeof body.isOnline === "boolean" ? body.isOnline : true,
      updatedAt: now,
    });
  } else {
