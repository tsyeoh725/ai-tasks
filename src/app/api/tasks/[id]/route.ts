...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (updates.assigneeId && updates.assigneeId !== existing.assigneeId && updates.assigneeId !== user.id) {
  ...
  if (updates.assigneeId !== undefined && updates.assigneeId !== existing.assigneeId) {
    await evaluateRules(existing.projectId, { type: "assigned", taskId: id, data: { assigneeId: updates.assigneeId || "" } });
  }
