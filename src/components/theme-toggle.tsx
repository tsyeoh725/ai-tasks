...
export function ThemeToggle({ compact = false }: { compact?: boolean }) {
  // Start with a stable default that matches SSR. The real value is loaded from
  // localStorage in the mount effect below — reading localStorage in the
  // useState initializer would cause a hydration mismatch (server has no
  // localStorage and falls back to a different value than the client).
  const [theme, setTheme] = useState<Theme>("light");
  const [mounted, setMounted] = useState(false);

  // Hydrate theme from localStorage and re-sync the .dark class
  useEffect(() => {
    const stored = window.localStorage.getItem(STORAGE_KEY) as Theme | null;
    // eslint-disable-next-line react-hooks/set-state-in-effect -- mount-only hydration from localStorage; useState initializer would cause SSR/CSR mismatch
  ...
  if (compact) {
    ...
    return (
      <button
        ...
        title={mounted ? `Theme: ${theme} (click for ${next})` : "Toggle theme"}
