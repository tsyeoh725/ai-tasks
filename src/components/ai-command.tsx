...
import { DefaultChatTransport, type UIMessage, type UIMessagePart, type UIDataTypes, type UITools } from "ai";
import { useEffect, useRef, useState, useMemo } from "react";
import { Button } from "@/components/ui/button";
import { cn } from "@/lib/utils";

type ChatUIPart = UIMessagePart<UIDataTypes, UITools>;
            ...
            >
              {message.role === "assistant" ? (
                <div className="space-y-2">
                  {message.parts?.map((part: ChatUIPart, i: number) => {
                    ...
                    if (part.type?.startsWith("tool-") || part.type === "dynamic-tool") {
                      const toolName = part.type === "dynamic-tool"
                        ? (part as Extract<ChatUIPart, { type: "dynamic-tool" }>).toolName
                        : part.type.replace("tool-", "");
                      const toolPart = part as Extract<ChatUIPart, { type: `tool-${string}` | "dynamic-tool" }>;
                      const state = "state" in toolPart ? (toolPart.state as string) : "";
                ...
                <p className="whitespace-pre-wrap">
                  {message.parts
                    ?.filter((p: ChatUIPart): p is Extract<ChatUIPart, { type: "text" }> => p.type === "text")
                    .map((p) => p.text)
                    .join("") || ""}
                </p>
              )}
...
function ToolCallDisplay({ toolName, state, result }: { toolName: string; state: string; result: unknown }) {
  ...
  const resultHasError = isDone && typeof result === "object" && result !== null && "error" in result && (result as { error?: unknown }).error;
  const isError = state === "error" || !!resultHasError;
